// game.js PART 1 / 5  (FIXED)
// CORE / STATE / INPUT / RACE FLOW
// VERSION: v1.2.0-fix-rails-pipes-rank
// ※このPART1の末尾に PART2→PART5 を順に追記
...
/* =======================
   VERSION
======================= */
const VERSION = "v1.2.0-fix-rails-pipes-rank";
...
const btnNext = document.getElementById("btnNext"); // リザルト用
...
function updateRank(){
  ...
  state.rankText=`RANK ${better+1}/${total}`;
}
...
/* === PART2 START === */
 // game.js PART 2 / 5  (FIXED)
// WORLD / SPAWN / NON-OVERLAP RULES / PLAYER-ONLY RINGS
...
function addRail(x){
  ...
  const h = Math.floor(world.groundH * 0.38); // 低め
  const w = img ? Math.floor(img.width * (h / img.height)) : 140;

  if(overlapsAny(x,w,world.pipes,60)) {
    world.nextRailX = x + w + 120;
    return;
  }

  world.rails.push({ x, y: world.groundTop - h, w, h });
  world.nextRailX = x + w + rand(200, 420);
}
...
function addPipe(x){
  ...
  const h = Math.floor(world.groundH * 0.38); // レールと同等
  const w = Math.floor(img.width * (h / img.height));

  if(overlapsAny(x,w,world.rails,60)) {
    world.nextPipeX = x + w + 200;
    return;
  }

  world.pipes.push({ x, y: world.groundTop - h, w, h, img });
  world.nextPipeX = x + w + rand(600, 800);
}
...
function addPuddle(x){
  ...
  world.nextPuddleX = x + w + rand(200, 360);
}
...
/* === PART3 START === */
 // game.js PART 3 / 5  (FIXED)
// PHYSICS / COLLISION
//  - ガードレールに乗れる
//  - ハーフパイプに安定して乗れる（端は床扱い）
...
/* === PART4 START === */
 // game.js PART 4 / 5  (FIXED)
// RENDER / HUD / RANK / MINIMAP / RESULT UI
...
function drawHUD(){
  ...
  ctx.fillText(state.rankText, CONFIG.LOGICAL_W-90, 22);
...
function drawResult(){
  ...
  for(let i=0;i<list.length;i++){
    ...
    y+=18;
    if(y>CONFIG.LOGICAL_H-60) break;
  }

  btnNext && (btnNext.style.display="block");
}
...
/* === PART5 START === */
 // game.js PART 5 / 5  (FIXED)
// LOOP / NEXT-RACE BUTTON / BOOT (FINAL)
